...
const EnhancedHeader = ({ userRole, onRoleChange, onNavigate }) => {
  const [isSearchOpen, setIsSearchOpen] = useState(false)
  const [searchQuery, setSearchQuery] = useState('')

  const navigationItems = [
    { id: 'dashboard', label: 'Dashboard', icon: '📊', path: '/admin' },
    { id: 'events', label: 'Event Curation', icon: '🎯', path: '/admin/events' },
    ...
    { id: 'pipeline', label: 'Data Pipeline', icon: '🔄', path: '/pipeline' }
  ]
          ...
          <div className="grid grid-cols-3 gap-2">
            {navigationItems.slice(0, 6).map((item) => (
              <button
                key={item.id}
                onClick={() => onNavigate(item.path)}
                className="flex flex-col items-center space-y-1 p-2 rounded-lg text-xs font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-all duration-200"
              >
